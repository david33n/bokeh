...
export type RefMap = Map<string, HasProps> // identifier -> object
...
export class Document {
  ...
  static _initialize_references_json(references_json: Struct[], old_references: RefMap, new_references: RefMap): void {
    const to_update: Map<string, [HasProps, Attrs, boolean]> = new Map()
    for (const obj of references_json) {
      const obj_id = obj.id
      const obj_attrs = obj.attributes

      const was_new = !old_references.has(obj_id)
      const instance = (!was_new ? old_references.get(obj_id) : new_references.get(obj_id))!

      // replace references with actual instances in obj_attrs
      const resolved_attrs = Document._resolve_refs(obj_attrs, old_references, new_references)
      to_update.set(instance.id, [instance, resolved_attrs as Attrs, was_new])
    }
    ...
    function foreach_depth_first(items: typeof to_update, f: Fn): void {
      const already_started: Set<HasProps> = new Set()
      function foreach_value(v: unknown) {
        if (v instanceof HasProps) {
          ...
          if (!already_started.has(v) && items.has(v.id)) {
            already_started.add(v)
            const [, attrs, was_new] = items.get(v.id)!
      ...
      for (const [instance] of items.values()) {
